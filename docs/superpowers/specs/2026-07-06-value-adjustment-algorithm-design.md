...
- Each `coef[d]` is a **backend constant** in a `SCORING_COEF` table, set from domain intuition
  (e.g. "+0.5 PPR-TE ⇒ +8% TE value") and tuned after the first real draft. Never user-facing.
...
- `SCORING_COEF` — per-dimension sensitivity coefficients.
...
- `SCARCITY_BAND`, `SCORING_BAND`, `CONCENTRATION_BAND`.

Initial values are seeded from domain intuition and sanity-checked against the identity case and a few
known scenarios (2-TE premium ⇒ elite TE up; 10-team start-9 ⇒ elite QB/RB up, depth down).
...
- **Caps:** each force stays within its band under extreme settings; adjusted budget ≥ 1, floor ≥ 5.
