...
- `computeSpreads(players)` → returns players annotated with `spread`, `strategyTag`, `spreadDynRank`, `spreadProjRank` (position-relative percentile rank gap + age-aware 2×2 tag); `strategyTagReason(tag)` for the modal copy; shared `formatSpread`/`spreadColor` display helpers
- Calibration in `src/lib/valueSpread.constants.ts` (`SPREAD_GATE`, backend-only, TUNABLE)
...
- **Value Spreads** — advisory overlay on the value sheet (successor to the removed strategy "Lens", which was pulled for mutating auction values). Never mutates values. `computeSpreads` (`src/lib/valueSpread.ts`) runs server-side in `src/app/draft/[draftId]/page.tsx`, annotating each `Player` with a position-relative percentile **rank gap** (`spread = pctProj − pctDyn` over the above-replacement common set, `vor > 0`) between projection value and dynasty value, plus an **age-aware 2×2 archetype tag** (`WIN-NOW`/`BARGAIN`/`FUTURE`/`FADE`) that fires only past a gate (`SPREAD_GATE = 15`, `valueSpread.constants.ts`) and only in the young/old age bands. Surfaced as: a sortable "Spread" column (nulls sort last) + archetype filter chips (pure view filter, hidden when no player has a tag) on `/` (value sheet), and a label in the bid modal's Price-context panel. No-read (spread `—`, no tag) for below-replacement / no-projection / non-QBRWTE players; prime/aging bands show the number but no tag. Gracefully dormant when a draft has no projections applied. Introduced `src/lib/ageBands.ts` (per-position age bands) and retrofitted `src/lib/ageColor.ts` onto it — a position-less caller (team avg age in `DossierFace`) keeps the global fallback. Spec: `docs/superpowers/specs/2026-07-14-value-spreads-design.md`.
