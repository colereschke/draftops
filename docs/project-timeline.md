...
| `FEAT-012`            | Sleeper-assisted trade prefill                          | Feature                     | Planned                      | Live-draft operations   | `FEAT-009`, `FEAT-010`              | —                                                          | Validate Sleeper transaction payloads/idempotency, then write a focused spec |
...
- Status: Planned.
- Goal: detect Sleeper `draft_picks` transfers and pre-fill the existing DraftOps trade flow with
  mapped teams and exact origin/year/round pick identity.
- Constraint: Sleeper provides no DraftOps budget amount. The operator must enter that amount and
  confirm; detection must never auto-create a trade.
- Dependencies: the shipped Sleeper roster-to-team mapping (`FEAT-009`) and budget-for-picks ledger
  (`FEAT-010`).
- Next action: validate transaction payloads, polling and deduplication behavior, then write a
  focused design spec.
...
- Validate Sleeper transaction payload stability and deduplication requirements before designing
  `FEAT-012`; keep it as operator-confirmed prefill rather than an automatic trade importer.
