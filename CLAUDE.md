...
│   ├── draft/
│   │   ├── not-found.tsx             # Tailored not-found for the /draft/* subtree — must live here, not in [draftId]/, since only a *parent* segment's not-found.tsx can catch notFound() thrown by [draftId]/layout.tsx
│   │   └── [draftId]/                # Value sheet plus budget, nominate, and teams subpages
│   │       ├── loading.tsx           # Route-segment loading state (RouteLoading)
│   │       ├── error.tsx             # Route-segment error boundary (RouteErrorBoundary)
│   │       ├── budget/loading.tsx    # Same pattern per subpage
│   │       ├── budget/error.tsx
│   │       ├── teams/loading.tsx
│   │       ├── teams/error.tsx
│   │       ├── nominate/loading.tsx
│   │       └── nominate/error.tsx
...
├── components/
│   ├── RouteLoading.tsx               # Shared loading.tsx body (spinner + label) for every /draft/[draftId]/* route segment
│   ├── RouteErrorBoundary.tsx         # Shared error.tsx body (incident ID + Try Again) for every /draft/[draftId]/* route segment
│   ├── AuctionSheet/                 # Main player value sheet + bid logging
│   │   └── urlState.ts               # parseAuctionSheetSearchParams/buildAuctionSheetQueryString — pos/strategy/search/sort/availableOnly, allowlist-validated
...
│   │   └── urlState.ts               # parseNominationPosFilter/buildNominationQueryString — position filter, allowlist-validated
...
│   │   └── urlState.ts               # sort + selected-team URL parse/serialize, allowlist-validated
...
│   ├── useUrlQuerySync.ts            # Mirrors a composed query string into the URL via history.replaceState (never pushState) without triggering navigation
│   ├── useDebouncedValue.ts          # Generic debounce hook — delays a value by delayMs before it's used downstream (e.g. search-to-URL sync)
...
- **Accessibility and contrast hardening (HARD-015)** — `src/lib/contrastRatio.ts` (`contrastRatio(hexA, hexB)`, WCAG 2.x relative-luminance formula) backs a token-contrast regression test (`src/lib/__tests__/tokenContrast.test.ts`) that caught `--text-muted` and `--age-old` (aliased to `--destructive`) falling short of the 4.5:1 AA text minimum; both tokens were retuned to pass against every background they're rendered on. `src/components/SkipLink.tsx` renders a visually-hidden-until-focused "Skip to main content" link (`href="#main-content"`), added once to the shared layout; every route now exposes exactly one `<main id="main-content">` landmark as the skip target, each also carrying `tabIndex={-1}` so Safari — which only scrolls on skip-link activation instead of moving focus like Chrome/Firefox — still lands focus on the landmark instead of leaving it in the nav. Placeholder-only search inputs (value sheet, teams, nominate) gained explicit accessible names (`aria-label`/associated `<label>`) since a placeholder alone doesn't satisfy screen readers. `PlayerTable`'s row had redundant non-semantic keyboard handling (a manual `onKeyDown` reimplementing activation on a non-interactive element) removed in favor of the row's existing labeled nested `<button>`, which already gets native keyboard support for free. `MutationStatus` (HARD-010's shared `aria-live="polite"` region) is now wired into `BudgetRefresher`, `RankingsUploadForm`, and `SleeperRosterSyncDialog` so their async outcomes are announced the same way bid/nomination mutations are. `Button`/`Toggle` gained a `size="touch"` variant (44px, the WCAG 2.5.5/2.5.8 comfortable touch-target minimum), applied throughout `BidModal`'s controls. `transition-all` was replaced with property-specific transitions on `Button`/`Toggle` (`src/components/ui/__tests__/transitions.test.ts` asserts this), and `globals.css` gained a global `@media (prefers-reduced-motion: reduce)` rule zeroing `transition-duration`, `animation-duration`, `animation-iteration-count`, and forcing `scroll-behavior: auto`. `jest-axe` was added as a dev dependency backing `src/__tests__/criticalRouteAccessibility.test.tsx` (an axe-driven accessibility smoke suite) alongside `criticalRouteLandmarks.test.tsx` (landmark/skip-link/keyboard-flow assertions), together covering the four critical routes (value sheet, budget, teams, nominate).
...
- **URL-synced filter/sort/search state and per-route loading/error states (HARD-018)** — the value sheet, teams page, and nominate page mirror their filter/search/sort/available-only/selected-team state into the URL query string via a shared `useUrlQuerySync` hook (`src/lib/useUrlQuerySync.ts`), which always calls `history.replaceState` — never `pushState` — so in-page filter clicks don't pollute browser back-button history; only page-to-page navigation does. Search input additionally runs through `useDebouncedValue` (`src/lib/useDebouncedValue.ts`) before it's written, so keystrokes don't spam the URL. Each component owns one pure `urlState.ts` parse/serialize module (`src/components/AuctionSheet/urlState.ts`, `src/components/RosterTracker/urlState.ts`, `src/components/NominationHelper/urlState.ts`) that allowlist-validates every URL param and falls back to its default on garbage or stale input — e.g. a bookmarked `?strategy=BARGAIN` against a draft with no projections applied is silently ignored rather than filtering the table to zero rows with no visible control to clear it. Every route segment under `/draft/[draftId]/` now has a tailored `loading.tsx` and `error.tsx` built from shared `RouteLoading`/`RouteErrorBoundary` components (`src/components/RouteLoading.tsx`, `src/components/RouteErrorBoundary.tsx`); `RouteErrorBoundary` reuses the existing `captureClientError`/incident-ID error reporting (`src/lib/clientObservability.ts`, `src/lib/incident.ts`). `src/app/draft/not-found.tsx` handles the whole `/draft/*` subtree's not-found case — it must live there rather than inside `[draftId]/`, since Next.js only lets a _parent_ segment's `not-found.tsx` catch a `notFound()` thrown by a layout in the segment below it, and `[draftId]/layout.tsx` throws `notFound()` itself. Plan: `docs/superpowers/plans/2026-07-21-hard-018-url-state.md`.
