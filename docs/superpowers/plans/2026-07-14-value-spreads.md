...
Then nest the spread block **inside** the existing `hasProjectionContext` panel — after the 3-column grid's closing `</div>` (`BidModal.tsx` line ~131) and before that panel `<div>`'s own closing `</div>` (line ~132) — so it reads as a footer row within the same card (the spec's single-panel layout, which avoids a second bordered box and a duplicated "Dynasty" label). Because a non-null `spread` implies `vor > 0`, which implies a non-null `projectionAuctionValue`, `hasProjectionContext` is always true whenever `player.spread != null`, so nesting is safe:
