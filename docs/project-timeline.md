...
- Active initiative: `FEAT-010` budget-for-picks trading.
...
| `FEAT-010`            | Budget-for-picks trading                                | Feature                     | In progress                  | Live-draft intelligence | `FEAT-003`, `FEAT-005A`, `FEAT-008` | Design/spec #10; current model work at `741652c`           | Complete trade entry, validation, persistence, and UI                    |
...
`FEAT-010` is the active initiative. The data model and budget-delta seams are being established
for trades that exchange auction budget and future picks. The implementation must keep three
concepts separate:
...
The active design and implementation plan live in `docs/superpowers/` until the feature is merged
and its durable decisions are extracted into `docs/DECISIONS.md`.
...
- Status: In progress.
- Goal: record budget-for-pick trades without corrupting buying power, pick ownership, or audit
  history.
- Current state: trade, pick-asset, and trade-audit models are present; budget-delta calculation,
  pick-holder resolution, and mutation helpers have been developed around them.
- Remaining work: complete the user-facing trade flow, enforce all trade invariants, cover edits,
  deletion, restoration, and dependent pick protection, and verify the full budget/teams/value
  experience against real PostgreSQL.
- Design constraints: preserve the single-operator model; do not treat trades as auction wins;
  retain origin-team semantics for pick valuation.
- Next action: finish the implementation plan tasks and run the full quality/integration gate
  before review.
...
- Decide whether dynamic pick valuation needs richer posture signals before trading is complete.
